...
function appkitSrcFiles(?string $subdir = null): array
{
...
it('declares no class constants anywhere in src/', function (): void {
    foreach (appkitSrcFiles() as $file) {
...
it('guards every helper function with function_exists', function (): void {
    foreach (appkitSrcFiles('Helpers') as $file) {
...
it('backs every enum with int or string', function (): void {
    foreach (appkitSrcFiles('Enums') as $file) {
        expect(file_get_contents($file))->toMatch(
...
it('does not ship a script that writes src/Helpers', function (): void {
    $scripts = dirname(__DIR__, 2).'/scripts';
    if (! is_dir($scripts)) {
    ...
    foreach (glob($scripts.'/*.php') ?: [] as $file) {
        ...
        expect($source)->not->toContain("src/Helpers", "{$file} writes or targets src/Helpers");
        expect($source)->not->toContain('file_put_contents', "{$file} writes files; helpers are owned source");
    }
});
